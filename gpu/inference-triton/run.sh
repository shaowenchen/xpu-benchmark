...
# GPU Inference Test Runner with NVIDIA Triton Server
# Start Triton Server using nerdctl

set -e

# Default configuration
IMAGE_NAME="shaowenchen/xpu-benchmark:gpu-inference-triton"
CONTAINER_NAME="xpu-benchmark-gpu-inference-triton"
HOST_PORT=8000
CONTAINER_PORT=8000
...
while [[ $# -gt 0 ]]; do
    ...
    *)
        ...
        echo ""
        echo "Options:"
        echo "  --start              Start Triton Server"
        echo "  --stop               Stop Triton Server"
...
download_model() {
    local model_path="$1"
    local model_dir="model_repository"
    ...
    if git clone --depth 1 --single-branch "$model_path" "$target_dir"; then
        ...
        create_triton_config "$model_name" "$target_dir"
    else
...
create_triton_config() {
    ...
    echo "🔧 Creating Triton model configuration for $model_name..."
    
    # Create model configuration directory
    local config_dir="$model_dir/config"
    mkdir -p "$config_dir"
    
    # Create config.pbtxt
    cat > "$config_dir/config.pbtxt" << EOF
name: "$model_name"
platform: "vllm"
max_batch_size: 8
...
EOF
    ...
    echo "✅ Triton configuration created: $config_dir/config.pbtxt"
}
...
download_model_concurrent() {
    ...
    download_single_model() {
        local model_path="$1"
        local model_name=$(basename "$model_path")
        local target_dir="model_repository/$model_name"
...
start_service() {
    ...
    if nerdctl ps -a | grep -q "$CONTAINER_NAME"; then
        ...
        nerdctl run -d \
            --gpus all \
            --name $CONTAINER_NAME \
            --volume $(pwd)/model_repository:/opt/tritonserver/model_repository \
    ...
    echo "Container ID: $(nerdctl ps --format 'table {{.ID}}' | grep $CONTAINER_NAME)"
    echo "Triton Server URL: http://localhost:$HOST_PORT"
    echo "Health check: http://localhost:$HOST_PORT/v2/health/ready"
...
if [ "$MODEL_MODE" = true ]; then
    ...
    echo "=== GPU Inference Test Runner with NVIDIA Triton Server ==="
    echo "Please specify an action:"
    echo "  $0 --start              # Start Triton Server"
    echo "  $0 --stop               # Stop Triton Server"
